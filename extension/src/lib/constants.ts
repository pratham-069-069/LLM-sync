...
export const GEMINI_API_KEY: string = __GEMINI_API_KEY__ || 'missing-gemini-key';
...
if (DEBUG_MODE) {
  ...
  console.log(`[NexusMind] OpenRouter API Key loaded: ${OPENROUTER_API_KEY ? 'Yes' : 'No'}`);
  console.log(`[NexusMind] Gemini API Key loaded: ${GEMINI_API_KEY ? 'Yes' : 'No'}`);
}
